...
with sync_playwright() as pw:
    ...
    page.screenshot(path='/tmp/pw_pos_concluir.png')

    # Procura links que contenham "Redigir", "Expedir", "Ofício" etc
    for term in ['Redigir', 'Expedir', 'Ofício', 'Oficio', 'Gerar']:
        els = page.locator(f'a:has-text("{term}")').all()
        for el in els:
            if el.is_visible():
                print(f'  Link visivel: "{el.inner_text()}" href={el.get_attribute("href")}')
