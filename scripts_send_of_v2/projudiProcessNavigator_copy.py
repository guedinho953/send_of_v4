...
class ProcessoParser:
    ...
    def analisar_movimentacao(self, ato, observacao=None):
        ...
        dados_partes = self.extrair_partes(self.soup)
        ...
        padrao_audiencia = next((p for p in self.PADROES_AUDIENCIAS if p in texto), None)
        if padrao_audiencia:
            ...
            dados["categoria"] = "não disponibilizado no djen"
            dados["djen"] = False
            for item in dados_partes:
                if item["papel"] == "destinatário":
                    item["domicilio_cnj"] = False
        ...
        padrao_certidao = next((p for p in self.PADROES_CERTIDAO if p in texto), None)
        if padrao_certidao:
            dados_movs = self.extrair_movimentacoes()
            for item in dados_movs.values():
                obs = (item.get("observacao") or "").strip()
                docs = item.get("documentos", [])
                if not (obs or docs):
                    continue
                dados.update({
                    "categoria": "certidao",
                    "subcategoria": padrao_certidao,
                    "observacao": obs or item.get("observacao"),
                    "documentos": docs or item.get("documentos"),
                })
        ...
        if "decorrido" in texto:
            dados["categoria"] = "decurso de prazo"
            prazo = self.extrair_data_referencia(texto)
            if prazo:
                dados['prazo_dias_ev_ref'] = prazo
        ...
        if "expedid" in texto:
            ...
            dados["situacao_comunicacao"] = "devolvido"
        ...
        if padrao_mandado:
            ...
            for item in dados_movs.values():
                ...
                dados.update({
                    "meio_comunicacao": "mandado",
                    "situacao_comunicacao": self.PADROES_MANDADOS[padrao_mandado],
                    "observacao": obs or item.get("observacao"),
                    "documentos": docs or item.get("documentos"),
                })
