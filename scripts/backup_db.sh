...
# Salva dump SQL + comprime e mantém os 7 mais recentes.
# Flags --no-owner --no-privileges: dump portável (espelhamento/recuperação
# em qualquer Postgres, sem depender do usuário/owner original).
...
# Fazer o dump (portável)
docker exec pg_send_of pg_dump -U send_of -d sccj \
    --no-owner --no-privileges > "$DUMP_FILE" 2>/tmp/pg_backup_err.log
